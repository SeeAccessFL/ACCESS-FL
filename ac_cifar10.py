...
logging.basicConfig(
    ...
    format='%(message)s',
    handlers=[
        logging.FileHandler("output_ac_cifar10.log"),  # Log to a file
...
def federated_learning(global_model, client_data, num_rounds, participating_list, public_parameters, test_data):
    ...
    log_print(f"\n===== Starting ACCESS-FL on CIFAR for {num_rounds} Training Rounds =====")
...
df.to_csv("accuracy_costs_ac_cifar10.csv", index=False)

# # Read the CSV file back into a DataFrame
# df_output = pd.read_csv("accuracy_costs_ac_cifar10.csv")
