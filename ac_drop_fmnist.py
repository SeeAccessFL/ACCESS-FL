...
logging.basicConfig(
    ...
    format='%(message)s',
    handlers=[
        logging.FileHandler("output_ac_drop_fmnist.log"),  # Log to a file
...
global_model.load_weights('2nn.h5')
...
# total_parameters = calculate_total_parameters(global_model)
...
def federated_learning(global_model, client_data, num_rounds, participating_list, public_parameters, test_data):
    ...
    log_print(f"\n===== Starting ACCESS-FL (dropp out scenario) on Fashion-MNIST for {num_rounds} Training Rounds =====")
    ...
    for round_num in tqdm(range(num_rounds)):
        ...
        #end_dropoutcheck_time = time.time()
        #server_dropoutcheck_time = (end_dropoutcheck_time - start_dropoutcheck_time) * 1000
...
df.to_csv("accuracy_costs_ac_drop_fmnist.csv", index=False)

# Read the CSV file back into a DataFrame
df_output = pd.read_csv("accuracy_costs_ac_drop_fmnist.csv")
...
# plt.plot(range(1, len(accuracy_log)+1), accuracy_log, marker='o')
# plt.title('Federated Learning Accuracy Over Rounds', fontsize=15, fontname='Times New Roman')
# plt.xlabel('Round', fontsize=15, fontname='Times New Roman')
# plt.ylabel('Accuracy', fontsize=15, fontname='Times New Roman')
