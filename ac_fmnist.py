...
logging.basicConfig(
    ...
    format='%(message)s',
    handlers=[
        logging.FileHandler("output_ac_fmnist.log"),  # Log to a file
...
def federated_learning(global_model, client_data, num_rounds, participating_list, public_parameters, test_data):
    ...
    log_print(f"\n===== Starting ACCESS-FL on Fashion-MNIST for {num_rounds} Training Rounds =====")
    ...
    for round_num in tqdm(range(num_rounds)):
        ...
        # Check client dropout incident
        start_dropoutcheck_time = time.time()
        if len(masked_weights) != len(participating_list):
            log_print(f"Client dropout at round {round_num+1}")
            participating_list = [client_id for client_id, _ in enumerate(masked_weights)]

            # server broadcast participating list
        ...
        server_time += server_dropoutcheck_time + server_aggw_time
...
df.to_csv("accuracy_costs_ac_fmnist.csv", index=False)

# Read the CSV file back into a DataFrame
df_output = pd.read_csv("accuracy_costs_ac_fmnist.csv")
